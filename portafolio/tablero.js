...
/* ======================================================================
   18 · Carga de un export más reciente (CSV del Portafolio)
   ====================================================================== */

function parseaCSV(texto) {
  /* Lector tolerante: separador ';' o ',', comillas dobles con escape "" */
  texto = texto.replace(/^﻿/, '');
  ...
  var cab = filas[0].map(function (x) { return x.trim(); });
  return filas.slice(1).filter(function (f) { return f.some(function (x) { return x.trim(); }); })
    .map(function (f) {
      var o = {};
      cab.forEach(function (k, j) { o[k] = (f[j] || '').trim(); });
      return o;
    });
}
...
function normalizaPct(s) {
  var v = parseFloat(String(s).replace('%', '').replace('—', '').trim());
  return isFinite(v) ? Math.round(v) : null;
}
var RE_URL = /https?:\/\/[^\s<>"')]+/g;
function sacaUrls(s) { return (String(s || '').match(RE_URL) || []); }
function sinUrls(s) { return String(s || '').replace(RE_URL, '').trim().replace(/^[·\-.\s]+|[·\-.\s]+$/g, ''); }

/* Toma cualquiera de los nombres posibles de una columna. */
function col(fila, nombres) {
  for (var i = 0; i < nombres.length; i++) {
    for (var k in fila) if (sinAcentos(k) === sinAcentos(nombres[i])) return fila[k];
  }
  return '';
}

function cargaExports(archivos) {
  var pendientes = archivos.length, ini = null, act = null;
  archivos.forEach(function (f) {
    var lector = new FileReader();
    lector.onload = function () {
      var filas = parseaCSV(lector.result);
      if (!filas.length) { if (!--pendientes) aplica(); return; }
      var claves = Object.keys(filas[0]).map(sinAcentos).join('|');
      if (claves.indexOf('iniciativa') >= 0 && claves.indexOf('actividad') >= 0) act = filas;
      else if (claves.indexOf('pilar') >= 0 || claves.indexOf('etapa') >= 0) ini = filas;
      else if (claves.indexOf('actividad') >= 0) act = filas;
      else ini = filas;
      if (!--pendientes) aplica();
    };
    lector.onerror = function () { if (!--pendientes) aplica(); };
    lector.readAsText(f, 'utf-8');
  });

  function aplica() {
    if (!ini && !act) { avisa('No se reconoció el formato de los archivos.'); return; }
    var datos = { fuente: 'Export cargado desde tu equipo el ' + HOY.toLocaleDateString('es-BO'), iniciativas: [], actividades: [] };

    datos.iniciativas = (ini || []).map(function (r) {
      var nota = col(r, ['Resultado esperado', 'Resultado']);
      var enl = sacaUrls(nota).concat(sacaUrls(col(r, ['Enlace', 'Link', 'Documento', 'URL'])));
      var directo = col(r, ['Enlace', 'Link', 'Documento', 'URL']);
      if (directo && /^https?:/i.test(directo) && enl.indexOf(directo) < 0) enl.push(directo);
      return {
        id: parseInt(col(r, ['ID']), 10) || 0, nombre: col(r, ['Nombre', 'Iniciativa']),
        pilar: col(r, ['Pilar']), estado: col(r, ['Estado']), etapa: col(r, ['Etapa']),
        impacto: col(r, ['Impacto']), avance: normalizaPct(col(r, ['Avance'])),
        ini: normalizaFecha(col(r, ['Fecha inicio'])), fin: normalizaFecha(col(r, ['Fecha fin planificada', 'Fecha fin'])),
        tipo: col(r, ['Tipo de elemento', 'Tipo']), area: col(r, ['Área impactada', 'Area impactada', 'Área']),
        macro: col(r, ['Macroproceso']), compl: col(r, ['Complejidad']), prio: col(r, ['Prioridad']),
        lider: col(r, ['Responsable líder', 'Responsable lider', 'Responsable']),
        equipo: col(r, ['Equipo responsable']), pc: col(r, ['Responsable PC']),
        cap: col(r, ['Requiere capacitación']) === 'Sí', eco: col(r, ['Impacto económico']) === 'Sí',
        nota: sinUrls(nota), enlaces: enl.filter(function (u, j, a) { return u && a.indexOf(u) === j; }),
        creado: normalizaFecha(col(r, ['Creado'])), registro: col(r, ['Registró', 'Registro'])
      };
    }).filter(function (i) { return i.id && i.nombre; });

    datos.actividades = (act || []).map(function (r) {
      var com = col(r, ['Comentarios', 'Comentario']);
      var bruto = col(r, ['Iniciativa']);
      var enl = sacaUrls(com).concat(sacaUrls(col(r, ['Actividad'])));
      var directo = col(r, ['Enlace', 'Link', 'Documento', 'URL']);
      if (directo && /^https?:/i.test(directo) && enl.indexOf(directo) < 0) enl.push(directo);
      return {
        id: parseInt(col(r, ['ID']), 10) || 0, nombre: sinUrls(col(r, ['Actividad'])),
        iniRaw: bruto, iniId: parseInt(String(bruto).split('·')[0], 10) || null,
        origen: col(r, ['Origen']), resp: col(r, ['Responsable']), area: col(r, ['Área', 'Area']),
        compromiso: normalizaFecha(col(r, ['Fecha compromiso', 'Fecha de compromiso'])),
        estado: col(r, ['Estado']), prio: col(r, ['Prioridad']), avance: normalizaPct(col(r, ['Avance'])),
        vencida: col(r, ['Vencida']) === 'Sí', comentario: sinUrls(com),
        enlaces: enl.filter(function (u, j, a) { return u && a.indexOf(u) === j; }),
        creado: normalizaFecha(col(r, ['Creado'])), registro: col(r, ['Registró', 'Registro'])
      };
    }).filter(function (a) { return a.id && a.nombre; });

    if (!datos.iniciativas.length) datos.iniciativas = window.PORTAFOLIO.iniciativas;
    if (!datos.actividades.length) datos.actividades = window.PORTAFOLIO.actividades;

    prepara(datos);
    montaFiltros(); sincronizaControles();
    el('pieFuente').textContent = datos.fuente;
    pinta();
    avisa('Datos actualizados: ' + datos.iniciativas.length + ' iniciativas y ' + datos.actividades.length + ' actividades.');
  }
}

function avisa(txt) {
  var a = h('div');
  a.style.cssText = 'position:fixed;left:50%;bottom:22px;transform:translateX(-50%);z-index:80;' +
    'background:var(--tinta);color:#fff;padding:11px 18px;border-radius:11px;font-size:12.5px;' +
...
function arranca() {
  if (!window.PORTAFOLIO) { document.body.appendChild(estadoVacio('No se pudo cargar datos.js.')); return; }
  prepara(window.PORTAFOLIO);
  el('pieFuente').textContent = window.PORTAFOLIO.fuente || '';
  montaFiltros();
  montaTema();
  ...
  el('btnCargar').addEventListener('click', function () { el('archivoCsv').click(); });
  el('archivoCsv').addEventListener('change', function () {
    if (this.files && this.files.length) cargaExports(Array.prototype.slice.call(this.files));
    this.value = '';
  });
